...
def list_claim():
    rch = dbh.reimburse_claims
    claim_list = rch.find({"username" : session["username"]}).sort('date', ASCENDING)
...
def list_all_claim():
    rch = dbh.reimburse_claims
    claim_list = rch.find().sort('date', ASCENDING)
...
def verify_claim():
    rch = dbh.reimburse_claims
    claim_list = rch.find({"status" : "submitted"}).sort('date', ASCENDING)
...
def approve_claim():
    rch = dbh.reimburse_claims
    claim_list = rch.find({"status" : "verified"}).sort('date', ASCENDING)
...
def pay_claim():
    rch = dbh.reimburse_claims
    claim_list = rch.find({"status" : "approved"}).sort('date', ASCENDING)
